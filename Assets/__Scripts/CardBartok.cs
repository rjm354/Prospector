...
{
    //These static fields are used to set values that wil be the same
    ...
    {
        //Make new interpolation lists for the new card
	...
	void Update ()
    {
	    switch(state)
        {
            //All the to____ states are ones where the card is interpolationg
            case CBState.toHand:
            ...
            case CBState.to:
                ...
                else if (u >= 1) //If u >= 1, we are finished moving
                {
                    ...
                    if (reportFinishTo != null)  //If there's a callback GameObject then use
                                                 //SendMessage to call the CBCallback method with this as
                                                 //the parameter.
                    {
